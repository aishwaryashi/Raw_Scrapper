...
// ─── Local ZIP-to-MSA database (built by scripts/build_zip_msa.js) ───────────
const __dirname = dirname(fileURLToPath(import.meta.url));
let _zipToMsa = null;
function getZipToMsa() {
  if (!_zipToMsa) {
    try {
      const raw = readFileSync(join(__dirname, '../data/zip_to_msa.json'), 'utf8');
      _zipToMsa = JSON.parse(raw);
    } catch {
      _zipToMsa = {};
    }
  }
...
// ─── Local ZIP lookup (zipcodes-us, offline, US only) ────────────────────────

/**
 * Instant offline lookup via the zipcodes-us flat-file database.
 * Returns structured geo data for valid US ZIP codes with no network call.
 */
function geocodeByZipLocal(zipCode) {
  if (!zipCode) return null;
  const result = zipFind(zipCode);
  if (!result || !result.isValid || !result.latitude) return null;

  const county = result.county ? `${result.county} County` : null;
  log.info(`[ZIP-LOCAL] ${zipCode} → city=${result.city}, state=${result.stateCode}, county=${county}, lat=${result.latitude}`);
  return {
    lat:             result.latitude,
    lng:             result.longitude,
    city:            result.city            || null,
    locality:        null,                         // neighbourhood — resolved by reverse geocode
    district:        county,                       // e.g. "Mecklenburg County"
    state:           result.state          || null,
    stateCode:       result.stateCode      || null,
    country:         'United States',
    countryCode:     'US',
    zipCode,
    formattedAddress: result.city && result.stateCode
      ? `${result.city}, ${result.stateCode}, ${zipCode}, United States`
      : null,
  };
}
...
async function geocodeByZipNominatim(zipCode, countryCode) {
  ...
  try {
    ...
    const geo = {
      ...
      locality:        addr.neighbourhood || addr.suburb || addr.city_district || addr.quarter || addr.village || null,
...
// ─── MSA / metro-area lookup — local ZIP database (offline, no API call) ─────

/**
 * Look up the metro area name for a US ZIP code using the pre-built
 * data/zip_to_msa.json database (generated by scripts/build_zip_msa.js).
 * Instant and works offline — no network call needed.
 */
function resolveMetroAreaFromZip(zipCode) {
  if (!zipCode) return null;
  const msa = getZipToMsa()[zipCode] || null;
  if (msa) log.info(`[METRO] zip=${zipCode} → "${msa}"`);
  return msa;
}
...
async function getLocalityFromCoords(lat, lng) {
  ...
  try {
    ...
    // Cascade through increasingly broad administrative levels so suburban/rural
    // coordinates still get a usable locality label.
    const locality =
      addr.neighbourhood   ||
      addr.suburb          ||
      addr.city_district   ||
...
async function enrichLocationGeo(location, apiKey) {
  ...
  // Step 1: instant offline lookup (US only, no network call).
  let geo = null;
  const isUS = cc.toUpperCase() === 'US';
  if (isUS) {
    geo = geocodeByZipLocal(zip);
  }

  // Step 2: Google Geocoding — used when local lookup has no result (non-US or unknown ZIP).
  if (!geo && apiKey) {
    geo = await geocodeByZipGoogle(zip, cc, apiKey);
    if (!geo) log.warning(`[GEOCODE] Google returned no result for zip=${zip} — falling back to Nominatim.`);
  }

  // Step 3: Nominatim — free fallback when neither local nor Google works.
  if (!geo) {
  ...
  // Final fallback: use city name when no neighbourhood-level name can be found.
  if (!location.locality) {
    location.locality = location.city || geo.city || null;
    if (location.locality) log.info(`[LOCALITY] No neighbourhood found — falling back to city: "${location.locality}"`);
  }

  // metroArea (US only): instant offline ZIP-to-MSA lookup.
  const resolvedIsUS = (geo.countryCode || cc).toUpperCase() === 'US';
  if (resolvedIsUS && (!location.metroArea || location.metroArea === 'not_found')) {
    const resolvedZip = location.zipCode || geo.zipCode || zip;
    const metro = resolveMetroAreaFromZip(resolvedZip);
...
function extractLocation(ad, raw, $) {
  ...
  const result = {
    ...
    nearbyNeighborhoods: coalesce(
      deepGet(ad, ['nearbyNeighborhoods']),
      deepGet(locObj, ['nearbyNeighborhoods']),
    ),
