 ...
 * oldschema.js
 *
 * LEGACY Firestore document schema — kept for reference / rollback only.
 * This is the exact `firestore.js` implementation that was in place BEFORE
 * the migration to the flat schema (see schema/ad-output-schema.json) that
 * splits ads into a "list" shape (propertyDetails + preferences) vs a
 * "find" shape (roomDetails + seeker).
 *
 * NOT imported anywhere in the active pipeline. `src/firestore.js` now
 * builds documents per the new schema.
 */
...
/**
 * Transform the scraped adData object into the LEGACY Firestore document structure.
 */
